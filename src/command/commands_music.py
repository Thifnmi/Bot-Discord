...
class Music(commands.Cog):
        ...
        if len(self.music_queue) > 0:
            if skip_to:
                if skip_to > len(self.music_queue):
                    await ctx.send(f"Out if range, please select >= {len(self.music_queue) + 1}")
                else:
                    self.is_playing = True
            ...
            self.is_playing = False
            await ctx.send("No song in queue. Please request to listen")
    ...
    async def skip(self, ctx, *args):
        if args:
            if len(args) > 1:
                await ctx.send("Skip to only receive one agrument. Example: `!skip 1`")
            to = args[0]
            self.vc.stop()
